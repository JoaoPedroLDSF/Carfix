...
def enviar_email(destinatario, assunto, corpo):
    remetente = "carfixofc@gmail.com"
    senha = "bpwojquxgsqhgkng"  # De preferência, use variável de ambiente
    msg = MIMEMultipart()
    msg["From"] = remetente
    msg["To"] = destinatario
    msg["Subject"] = assunto
...
@app.route("/esquecer/<token>", methods=["GET", "POST"])
def esquecer(token=None):
    s = URLSafeTimedSerializer(app.secret_key)
    ...
    if token:
        ...
        try:
            ...
            flash("O link expirou. Solicite a redefinição novamente.")
            return redirect(url_for('esquecer'))
        except BadSignature:
            flash("O link é inválido.")
            return redirect(url_for('esquecer'))

        if request.method == "POST":
            nova_senha = request.form.get('senha')
            confirma_senha = request.form.get('senha_confirm')
            ...
            cursor.execute('UPDATE usuario SET senha = ? WHERE email = ?', (senha_hash, email))
            db.commit()
    ...
    # Sem token: página para inserir e-mail
    if request.method == "POST":
        email = request.form.get('email')
        ...
        if usuario:
            ...
            link = url_for('esquecer', token=token, _external=True)
            assunto = "Redefinir sua senha"
            corpo = f"Olá, clique no link abaixo para redefinir sua senha:\n\n{link}\n\nEste link expira em 1 hora."
            enviar_email(email, assunto, corpo)

        # Aqui NUNCA usar redirect, só renderizar com flash
        flash('Se o e-mail estiver cadastrado, você receberá as instruções para redefinir a senha. Verifique sua caixa de entrada e spam.')
        return render_template('esquecer.html')

    # GET simples, só mostra o formulário
    return render_template('esquecer.html')
